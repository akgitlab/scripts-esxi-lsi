...
# If the controller id is not passed, we display all the information
    if [[ $CTLID = "" ]]; then
       echo "ssh -i /var/lib/zabbix/.ssh/id_rsa zabbix@$HOST '/opt/lsi/storcli/storcli /call/cv show all j'" | bash
    else
...
# We form the data of the received controller
        CTLSTR="ssh -i /var/lib/zabbix/.ssh/id_rsa zabbix@$HOST '/opt/lsi/storcli/storcli /c$CTLID/cv show all j'"
        echo $CTLSTR | bash
    fi
fi
