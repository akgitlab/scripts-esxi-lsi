...
# If the controller id is not passed, we display all the information
    if [[ $CTLID = "" ]]; then
       echo "ssh -i /usr/lib/zabbix/.ssh/id_rsa zabbix@$HOST 'cd /opt/lsi/storcli && ./storcli /call/vall show all j'" | bash
    else
...
# We form the data of the received controller
        CTLSTR="ssh -i /usr/lib/zabbix/.ssh/id_rsa zabbix@$HOST 'cd /opt/lsi/storcli && ./storcli /c$CTLID/vall show all j'"
        echo $CTLSTR | bash
    fi
fi
